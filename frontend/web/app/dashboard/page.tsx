  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end fade-in">
        ...
        <Link href={`/profile/${demoPassport.wallet}`} className="inline-flex items-center gap-2 border border-cyan/20 bg-panel/70 px-4 py-2 text-sm font-medium text-cyan hover-lift">
          ...
          <div className="flex h-full flex-col justify-between border border-white/15 bg-black/35 p-5 backdrop-blur">
            ...
            <div className="ascend">
              ...
              <div className="mt-6 h-4 overflow-hidden border border-cyan/25 bg-black/45">
                <div className="h-full bg-gradient-to-r from-cyan via-aqua to-green progress-fill" style={{ width: `${appProgress.percent}%` }} />
              </div>
        ...
        <section className="grid gap-5">
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={label as string} className="rune-panel p-4 scale-in-stagger hover-lift" style={{ "--index": i } as React.CSSProperties}>
          ...
          <div className="rune-panel p-5 fade-in-delay-3 hover-glow">
            ...
            <div className="mt-4 grid gap-3 font-mono text-sm">
              {["+500 XP · Deploy Contract verified", "ACH_001 · First Blood unlocked", "Stage 2 · Initiate ignition complete"].map((item, i) => (
                <div key={item} className="border border-cyan/10 bg-black/20 p-3 text-muted float-message" style={{ animationDelay: `${i * 100}ms` }}>{item}</div>
              ))}
            </div>
          </div>
        </section>
      ...
      <section className="mt-5 rune-panel p-5 fade-in-delay-2">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-5">
          {evolutionStages.map((stage, i) => (
            <div key={stage.id} className={stage.id === demoPassport.stage ? "border border-cyan/40 bg-cyan/10 p-4 shadow-rune scale-in-stagger hover-lift" : "border border-cyan/15 bg-black/20 p-4 scale-in-stagger hover-lift"} style={{ "--index": i } as React.CSSProperties}>
              <div className="flex items-center gap-2">
