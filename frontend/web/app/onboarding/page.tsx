    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-8 grid gap-3 md:grid-cols-3">
        {["Connect Wallet", "Choose Class", "Mint Passport"].map((step, index) => (
          <div key={step} className="rune-panel flex items-center gap-3 p-4">
      ...
      <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <section className="rune-panel h-fit p-6">
          <p className="font-mono text-xs uppercase tracking-[0.22em] text-cyan">Onboarding ritual</p>
          <h1 className="mt-3 text-4xl font-semibold">Mint your Soulbound Passport.</h1>
          ...
          <button className="rune-button mt-6 inline-flex items-center gap-2 px-4 py-3 font-semibold">
            <Wallet className="size-4" />
            Connect Wallet
          </button>
          <div className="mt-6 border border-cyan/15 bg-black/25 p-4">
        ...
        <section className="grid gap-3 md:grid-cols-2">
          {builderClasses.map((builderClass) => (
            <article key={builderClass.id} className={builderClass.id === 1 ? "rune-panel border-cyan/45 p-5 shadow-rune" : "rune-panel p-5"}>
              <div className="flex items-start justify-between gap-4">
                ...
                {builderClass.id === 1 ? <Check className="size-5 text-green" /> : null}
              </div>
          ...
          <Link href="/dashboard" className="rune-button inline-flex items-center justify-center px-4 py-3 font-semibold md:col-span-2">
