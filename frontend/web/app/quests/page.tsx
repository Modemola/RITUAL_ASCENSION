    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex gap-2 overflow-x-auto">
          {["All", "Class", "Completed", "Available"].map((item) => (
            <button key={item} className="border border-cyan/15 bg-panel/70 px-3 py-2 text-sm text-muted hover:text-cyan">{item}</button>
          ))}
        </div>
      </div>
      <div className="mt-5 flex items-center gap-3 border border-cyan/15 bg-panel/70 px-3 py-2">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {quests.map((quest) => {
          ...
          return (
            <article key={quest.id} className="rune-panel flex min-h-72 flex-col justify-between p-5 hover:border-cyan/45">
              <div>
              ...
              <div className="mt-5 border-t border-cyan/10 pt-4">
                ...
                <Link href={`/quests/${quest.id}`} className="mt-4 inline-flex w-full justify-center border border-cyan/25 bg-cyan/10 px-3 py-2 text-sm font-medium text-cyan hover:border-cyan/60">
                  Submit Proof
                </Link>
              </div>
            </article>
